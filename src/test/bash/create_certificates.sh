...
trap 'echo "[ERROR] create_certificates.sh: command failed at line ${LINENO}" >&2' ERR
...
elif [[ "${RUNS_ON_WINDOWS_OS}" == true ]] && [[ -n "${JAVA_HOME:-}" ]]; then
...
if [[ -z "${KEYTOOL}" ]] || [[ ! -x "${KEYTOOL}" ]]; then
  echo "[ERROR] No keytool in PATH/JAVA_HOME: ${KEYTOOL:-<empty>}"
  exit 1
fi
...
echo "[INFO] Certificate generation finished successfully"
