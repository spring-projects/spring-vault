...
`VaultTemplate` is the place to look for accessing functionality such as reading data from Vault or issuing administrative commands.
...
`VaultTemplate` is built on top of `RestTemplate` as low-level HTTP interface.
