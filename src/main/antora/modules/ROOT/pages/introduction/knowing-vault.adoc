...
Security and working with secrets are concerns of every developer working with databases, user credentials, or API keys.
Vault steps in by providing secure storage combined with access control, revocation, key rolling, and auditing.
...
The starting point for learning about Vault is https://www.vaultproject.io[www.vaultproject.io].
...
Vault can manage static and dynamic secrets such as application data and usernames/passwords for remote applications and resources, and provide credentials for external services such as MySQL, PostgreSQL, Apache Cassandra, Consul, AWS, and more.
