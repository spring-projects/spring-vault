...
An easy way to bootstrap a working environment is to create a Spring-based project in https://spring.io/tools/sts[STS].
...
To create a Spring project in STS, select File -> New ->
Spring Template Project -> Simple Spring Utility Project, and press Yes when prompted.
Then enter a project name and a package name such as `org.springframework.vault.example`.
...
If you are using a milestone or release candidate, you will also need to add the Spring Milestone repository to your Maven `pom.xml` at the same level as the `<dependencies/>` element.
...
The repository is also https://repo.spring.io/milestone/org/springframework/vault/[browsable here].

If you are using a SNAPSHOT, you will also need to add the Spring Snapshot repository to your Maven `pom.xml` at the same level as the `<dependencies/>` element.
...
The repository is also https://repo.spring.io/snapshot/org/springframework/vault/[browsable here].
