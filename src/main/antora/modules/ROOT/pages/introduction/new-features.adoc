...
* Support for versioned Key/Value secrets engines for Vault repositories.
...
* Add support for Jetty as a reactive HttpClient.
* `LifecycleAwareSessionManager` and `ReactiveLifecycleAwareSessionManager` now emit `AuthenticationEvent`s.
