...
A key aspect of reactive applications is the concept of backpressure, a mechanism
that ensures producers do not overwhelm consumers. For example, in a pipeline that extends
from the database to the HTTP response, if the HTTP connection is too slow, the data repository
can slow down or stop completely until network capacity becomes available.

[[vault.core.reactive.template.client]]
== Reactive Vault Client

Spring Vault's reactive client support is built on top of <<vault.authentication.steps,composable authentication steps>> and Spring's functional `WebClient` via Reactor Netty or Jetty, both of which provide fully non-blocking, event-driven HTTP clients.

It exposes javadoc:org.springframework.vault.authentication.VaultTokenSupplier[] as a supplier of javadoc:org.springframework.vault.support.VaultToken[] to authenticate HTTP requests
...
is the central class of Spring's reactive Vault support, providing a rich feature set to
...
As much as possible, the methods on javadoc:org.springframework.vault.core.ReactiveVaultOperations[] are named after methods
...
the use of the Vault API and javadoc:org.springframework.vault.core.ReactiveVaultOperations[]. A major difference between
the two APIs is that javadoc:org.springframework.vault.core.ReactiveVaultOperations[] accepts domain objects instead of
...
NOTE: The preferred way to reference the operations on a javadoc:org.springframework.vault.core.ReactiveVaultTemplate[] instance
is through its interface javadoc:org.springframework.vault.core.ReactiveVaultOperations[].

For functionality not explicitly exposed by javadoc:org.springframework.vault.core.ReactiveVaultTemplate[], you can use one of
several execution callback methods to access underlying APIs. The execution callbacks
...
javadoc:org.springframework.vault.core.ReactiveVaultTemplate[] and a `VaultTokenSupplier` registered inside a managed context will participate
...
the session token should be kept throughout a session.
...
is routed into one of the template classes' execution callback methods. This helps ensure
that exceptions are handled and that any required resource management is performed
consistently. While this was of much greater need in the case of JDBC and JMS
...
sequence with the given `WebClient`, allowing you to interact with Vault without a session context.

* `<T> T` *doWithSession* `(Function<WebClient, ? extends T> clientCallback)` Composes a reactive
sequence with the given `WebClient`, allowing you to interact with Vault in an authenticated session.
