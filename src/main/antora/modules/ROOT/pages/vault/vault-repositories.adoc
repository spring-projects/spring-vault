...
As of version 2.4, Spring Vault can additionally use the key/value version 2 secrets engine. The actual secrets engine version is discovered at runtime.

NOTE: Deletes within a versioned key/value secrets engine use the `DELETE` operation. Secrets are not destroyed through `CrudRepository.delete(…)`.
...
To access domain entities stored in Vault, you can leverage repository support that significantly eases their implementation.
...
Make sure to include the `spring-data-commons` and `spring-data-keyvalue` artifacts on your classpath.

The easiest way to achieve this is to set up dependency management and add the artifacts to your `pom.xml`:

Then add the following to the `<dependencies>` section of your `pom.xml`.
...
The component that glues these pieces together is the corresponding Spring configuration.
...
void basicCrudOperations() {
  ...
  creds.setAddress(new Address("308 Negra Arroyo Lane", "Albuquerque", "New Mexico", "87104"));
...
<1> Stores properties of `Credentials` inside a Vault hash with a key pattern `keyspace/id`, in this case `credentials/heisenberg`, in the key/value secrets engine.
...
Vault repositories store objects in Vault using JSON as an interchange format.
Object mapping between JSON and the entity is done by `VaultConverter`.
The converter reads and writes `SecretDocument` instances that contain the body of a `VaultResponse`. `VaultResponse`s are read from Vault and the body is deserialized by Jackson into a `Map` of `String` and `Object`.
...
<1> The `_class` attribute is included at the root level as well as on any nested interface or abstract types.
...
Those converters can take care of converting to and from types such as `LocalDate` and `SecretDocument`,
where the former is suitable for simple properties and the latter for complex types in their JSON representation.
The second option offers full control over the resulting `SecretDocument`.
Writing objects to `Vault`
will delete the content and re-create the whole entry, so unmapped data will be lost.
...
Vault has no query engine but requires direct access to HTTP context paths.
...
Query methods support sorting and paging by selecting an in-memory sublist (offset/limit) of IDs retrieved from a Vault context path.
Sorting is not limited to a particular field, unlike query method predicates.
...
Using paging and sorting requires secret fetching before filtering the IDs, which impacts performance.
Sorting and paging guarantee the same result even if the natural order of IDs returned by Vault changes.
Therefore, all IDs are fetched from Vault first, then sorting is applied and afterwards filtering and offset/limiting.
...
Vault's key/value secrets engine version 2 can maintain versioned secrets.
Spring Vault supports versioning through a version property in the domain model that is annotated with `@Version`.
...
repo.save(credentials);                                                          <3>


concurrent.setPassword("concurrent change");

repo.save(concurrent); // throws OptimisticLockingFailureException               <4>
...
The Key/Value version 2 secrets engine maintains versions of secrets that can be accessed by implementing https://docs.spring.io/spring-data/commons/docs/current/api/org/springframework/data/repository/history/RevisionRepository.html[`RevisionRepository`] in your Vault repository interface declaration.
