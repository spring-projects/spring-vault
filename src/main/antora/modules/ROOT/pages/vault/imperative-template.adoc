...
is the central class of Spring's Vault support, providing a rich feature set to
...
As much as possible, the methods on javadoc:org.springframework.vault.core.VaultOperations[] are named after methods
...
the use of the Vault API and `VaultOperations`. A major difference between
the two APIs is that `VaultOperations` accepts domain objects instead of
JSON Key-Value pairs.

Paths used in `VaultTemplate` (and interfaces accessible from there) are considered
relative to the `VaultEndpoint`. Fully qualified URIs can be used
...
NOTE: The preferred way to reference the operations on a javadoc:org.springframework.vault.core.VaultTemplate[] instance
is through its interface javadoc:org.springframework.vault.core.VaultOperations[].

While many convenience methods on `VaultTemplate` help you perform common tasks,
you can use one of several execution callback methods to access underlying APIs
when you need Vault functionality not explicitly exposed by `VaultTemplate`.
The execution callbacks give you a reference to a `RestOperations` object.
Please see the section <<vault.core.executioncallback,Execution Callbacks>> for more information.

Now let's look at some examples of how to work with Vault in the context of the Spring container.
...
provided by the Spring IoC container. This is useful to dispose of active Vault sessions upon
...
classes typically extend javadoc:org.springframework.vault.config.AbstractVaultConfiguration[] and are required to
provide additional details that are environment-specific.

Extending javadoc:org.springframework.vault.config.AbstractVaultConfiguration[] requires implementing
...
.Registering Spring Vault by applying injected properties
====
[source,java]
----
@Configuration
public class AppConfig extends AbstractVaultConfiguration {
    ...
    /**
     * Specify an endpoint that was injected as a URI.
     */
    @Override
    public VaultEndpoint vaultEndpoint() {
...
Spring Vault requires a `ClientAuthentication` to log in and access Vault.
See <<vault.core.authentication>> for details about authentication.
Vault login should not occur on each authenticated Vault interaction; instead,
the session token must be reused throughout a session. This aspect is handled by a
`SessionManager` implementation. A `SessionManager` decides how often to obtain a token
and whether to revoke or renew it. Spring Vault comes with two implementations:
...
renewal if a token is renewable and revokes a login token on disposal.
...
Spring Vault includes javadoc:org.springframework.vault.config.EnvironmentVaultConfiguration[] to configure the Vault client from Spring's `Environment` and a set of predefined
property keys.
...
Import javadoc:org.springframework.vault.config.EnvironmentVaultConfiguration[] into existing
Java-based configuration classes with `@Import(EnvironmentVaultConfiguration.class)` and supply configuration properties through any of Spring's `PropertySource`s.
...
** Enabled SSL/TLS protocols: `vault.ssl.enabled-protocols` (since 2.3.2, optional, protocols separated by commas)
** Enabled SSL/TLS cipher suites: `vault.ssl.enabled-cipher-suites` (since 2.3.2, optional, cipher suites separated by commas)
...
* UserId: `vault.app-id.user-id`. `MAC_ADDRESS` and `IP_ADDRESS` use the `MacAddressUserId` and `IpAddressUserId` user ID mechanisms, respectively.
...
One common design feature of all Spring template classes is that all functionality is routed into one of the template classes' execution callback methods.
This helps ensure that exceptions are handled and that any required resource management is performed consistently.
...
`RestOperationsCallback` and allows you to interact with Vault using `RestOperations` without requiring a session.

* `<T> T` *doWithSession* `(RestOperationsCallback<T> callback)` Executes the given
`RestOperationsCallback` and allows you to interact with Vault in an authenticated session.

Here is an example that uses the `RestOperationsCallback` to initialize Vault:
