...
| `4xx`/`5xx` status codes are translated to `HttpClientErrorException` and `HttpServerErrorException`, respectively; `onStatus` lets you register custom handlers.
...
| Convenience methods for setting tokens (`token(…)`) and namespaces (`namespace(…)`) make it easier to manage authentication headers. They can be used with javadoc:org.springframework.vault.authentication.ClientAuthentication[] implementations to log into Vault.
| Path-based request methods allow consuming Vault responses as javadoc:org.springframework.vault.support.VaultResponse[] through `ResponseSpec.body()` and `ResponseSpec.toEntity()` methods and support response wrapping directly.
| `4xx`/`5xx` status codes are translated to `VaultClientResponseException`; `onStatus` lets you register custom handlers.
...
Its reactive variant `ReactiveVaultClient` mirrors `VaultClient`'s design for non-blocking access.
Going forward, the documentation shows `VaultClient`. Except for the reactive aspects, `ReactiveVaultClient` works the same way.
It serves as an abstraction over HTTP libraries and handles conversion of HTTP request and response content to and from higher-level Java objects. It is built on top of `RestClient` or, for its reactive variant, `WebClient`.
...
For example, the response body can be accessed by using `retrieve().body(Class)` or `retrieve().body(ParameterizedTypeReference)` for parameterized types like lists. A convenience method `retrieve().body()` returns the body using Vault's default response type javadoc:org.springframework.vault.support.VaultResponse[].
The `body` method converts the response contents into various types – for instance, bytes can be converted into a `String`, and JSON can be converted into objects using Jackson.
...
If the response is not of interest for your use case, you can use `retrieve().toBodilessEntity()`.
