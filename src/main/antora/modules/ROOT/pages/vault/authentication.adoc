...
methods. Spring Vault supports multiple authentication mechanisms.
...
Disclosure of login credentials to any party allows them to log in to Vault and access secrets that
...
NOTE: Spring allows multiple ways to obtain `Environment`. When using `VaultPropertySource`, injection via `@Autowired Environment environment` does not provide the `Environment` because the `Environment` bean is still being constructed and autowiring occurs at a later stage. Your configuration class should rather implement `ApplicationContextAware` and obtain the `Environment` from `ApplicationContext`.
...
If a token is disclosed to an unintended party, it gains access to Vault and
...
The second token is the UserId, which is determined in part by the application
and usually related to the runtime environment. IP address, MAC address, or a
Docker container name are good examples. Spring Vault supports
IP address, MAC address, and static UserIds (e.g. supplied via System properties).
The IP and MAC addresses are represented as a hex-encoded SHA-256 hash.

IP address-based UserIds use the local host's IP address.
...
The corresponding command to generate the IP address UserId from the command line is:
...
MAC address-based UserIds obtain their network device from the
...
name or an interface index (0-based).
...
The corresponding command to generate the MAC address UserId from the command line is:

----
$ echo -n 0AFEDE1234AC | sha256sum
----

NOTE: The MAC address is specified in uppercase and without colons.
Including the line break of `echo` leads to a different hash value
so make sure to include the `-n` flag.

=== Custom UserId

A more advanced approach lets you implement your own `AppIdUserIdMechanism`.
...
Spring Vault supports AppRole authentication by providing a RoleId alone,
providing a RoleId and SecretId, or fetching the RoleId and SecretId from Vault
...
Spring Vault also supports full pull mode. If RoleId and SecretId are not provided,
...
does not require pre-deploying or provisioning security-sensitive
...
AWS-EC2 authentication enables a nonce by default to follow
...
During the first login, Spring Vault generates a nonce that is stored in the auth method alongside the instance ID.
...
configuration of credentials but can obtain them from their metadata sources.
...
.Using an AWS-EC2 instance profile as a credentials source
====
[source,java]
----
@Configuration
...
resource group name, VM name). These details can be configured through
...
calling GCP IAM's https://cloud.google.com/iam/docs/reference/credentials/rest/v1/projects.serviceAccounts/signJwt[`projects.serviceAccounts.signJwt`] API. The caller authenticates against GCP IAM and thereby proves its identity.
This Vault auth method treats GCP as a Trusted Third Party.

IAM credentials can be obtained from either the runtime environment
or supplied externally as JSON. JSON is the preferred form because it
...
NOTE: Google credentials require an OAuth 2 token to maintain the token lifecycle. All APIs
are synchronous; therefore, `GcpIamCredentialsAuthentication` does not support `AuthenticationSteps`, which is
required for reactive usage.

NOTE: `GcpIamCredentialsAuthentication` uses the https://cloud.google.com/iam/docs/reference/credentials/rest/v1/projects.serviceAccounts/signJwt[IAM Credentials API] and replaces the deprecated `GcpIamAuthentication`, which uses the deprecated https://cloud.google.com/iam/docs/reference/rest/v1/projects.serviceAccounts/signJwt[IAM API].
...
to pass the token to Vault, which then authenticates against your GitHub
account.
...
If the signature and any bound organization/space/application IDs match, Vault issues an appropriately-scoped token.

Instance credentials are available from the files specified by the `CF_INSTANCE_CERT` and
`CF_INSTANCE_KEY` environment variables.
...
The `cert` auth method allows authentication using SSL/TLS client
...
workflow. Cubbyhole authentication uses tokens as the primary login method.
...
Tokens retrieved from Cubbyhole that are associated with a non-zero TTL start their TTL at the
time of token creation. That time is not necessarily identical to application
...
retrieving the token. A self-lookup will look up renewability and the remaining TTL.
...
Since 0.8.3, Vault supports https://www.vaultproject.io/docs/auth/kubernetes.html[kubernetes]-based authentication using Kubernetes tokens.
...
Username/Password is typically an end-user authentication scheme.
Using username and password is supported by multiple Vault authentication methods:

* Username and Password (`userpass`)
* LDAP (`ldap`)
* Okta (`okta`, also supports time-based one-time tokens)
* RADIUS (`radius`)

`UserPasswordAuthenticationOptions` can be used with all the above-mentioned authentication methods as the Login API is similar across all mechanisms.
Ensure that you use the appropriate auth mount path when configuring `UserPasswordAuthenticationOptions`.
...
Composing authentication methods and reusing common steps, such as posting a login
payload to Vault or retrieving authentication input from an HTTP source, are not supported
by `ClientAuthentication` objects.

Authentication steps provide reusable common authentication activities.
...
multiple authentication steps start with a `Supplier` or `HttpRequest` that provides an
...
----
<1> Start by declaring `AuthenticationSteps` accepting a `Supplier<T>`.
...
Many `ClientAuthentication` implementations come with static factory methods to create `AuthenticationSteps`
...
Spring Vault provides javadoc:org.springframework.vault.authentication.LifecycleAwareSessionManager[], a session manager that can renew the token until it reaches its terminal TTL and then perform another login to obtain the next token associated with the session.

Depending on the authentication method, a login can create two kinds of tokens:

* javadoc:org.springframework.vault.support.VaultToken[]: A generic token that encapsulates the actual token.
* javadoc:org.springframework.vault.authentication.LoginToken[]: A token associated with renewability/TTL.

Authentication methods such as javadoc:org.springframework.vault.authentication.TokenAuthentication[] create only a `VaultToken` which does not carry any renewability/TTL details. javadoc:org.springframework.vault.authentication.LifecycleAwareSessionManager[] will run a self-lookup on the token to retrieve renewability and TTL from Vault.
`VaultToken` instances are renewed periodically if self-lookup is enabled. Note that `VaultToken` instances are never revoked; only `LoginToken` instances are revoked.

Authentication methods creating `LoginToken` directly (all login-based authentication methods) already provide all necessary details to set up token renewal. Tokens obtained from a login are revoked by `LifecycleAwareSessionManager` if the session manager is shut down.
