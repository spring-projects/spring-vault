...
Vault to store encrypted properties. Spring Vault supports Vault as a property
...
In the code above, javadoc:org.springframework.vault.core.env.VaultPropertySource[] was added with the highest precedence
in the search order. If it contains a `foo` property, it will be detected and returned
...
Spring Vault's `PropertySource` rotates generic secrets when their TTL is reached.
