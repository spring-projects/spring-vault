...
* Java's built-in `HttpClient` (default client if no other is available)
...
Please note that javadoc:org.springframework.vault.support.SslConfiguration[] can be used only when Apache Http Components is on your classpath.
