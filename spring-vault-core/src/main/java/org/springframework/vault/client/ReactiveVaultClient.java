 ...
 * {@link ReactiveVaultClient#builder()} to prepare an instance. To use the same configuration as
 * a {@link WebClient}, use {@link #builder(WebClient)}.
 * <p>
 * {@code ReactiveVaultClient} is intended to be used with relative paths requiring a
...
public interface ReactiveVaultClient {
	...
	/**
	 * Return a builder to create a new {@code ReactiveVaultClient} whose settings are replicated
	 * from this {@code ReactiveVaultClient}.
	 */
	Builder mutate();
	 ...
	 * {@link Builder#endpoint(VaultEndpoint)}.
	 *
	 ...
	 * For more details see {@link Builder#endpoint(VaultEndpointProvider)}.
	 *
	...
	/**
	 * Variant of {@link #create()} that accepts a default {@code ReactiveVaultEndpointProvider}.
	 * For more details see {@link Builder#endpoint(ReactiveVaultEndpointProvider)}.
	 *
	 * @param endpointProvider the endpoint provider for all relative path requests.
	 * @see #builder()
	 */
	...
	/**
	 * Obtain a {@code ReactiveVaultClient} builder based on the configuration of the given
	 * {@link WebClient}.
	 *
	 * @param webClient the {@link WebClient} to base the returned builder's
	 *                  configuration on
	 * @return a {@code ReactiveVaultClient} builder initialized with {@code webClient}'s
	 * configuration
	 */
	...
	interface Builder {
		 ...
		 * <li>{@link #endpoint(ReactiveVaultEndpointProvider)}}.
		 * </ul>
		 *
		...
		/**
		 * Provide a consumer to access {@link WebClient.Builder} with the possibility to
	...
	interface RequestHeadersSpec<S extends ReactiveVaultClient.RequestHeadersSpec<S>> {
		 ...
		 * {@link ReactiveVaultClient.ResponseSpec} to select from a number of built-in options to
		 * extract the response. For example:
		 *
		 * <p><pre class="code">
	...
	interface ResponseSpec {
		 ...
		 * {@link VaultClientResponseException} which is created with
		...
		/**
		 * Decode the body to {@link VaultResponse}. For an error response (status
		 * code of 4xx or 5xx), the {@code Mono} emits a {@link VaultException}.
		 ...
		 * code of 4xx or 5xx), the {@code Mono} emits a {@link VaultClientResponseException}.
		 * Use {@link #onStatus(Predicate, Function)} to customize error response
		 * handling.
		 *
		 ...
		 * emits a {@link VaultClientResponseException}. Use {@link #onStatus(Predicate, Function)}
		 ...
		 * {@link VaultClientResponseException}. Use {@link #onStatus(Predicate, Function)}
		 * to customize error response handling.
		 *
		 * @return the {@code ResponseEntity}.
		 */
	 ...
	 */
	interface RequestHeadersBodyPathSpec extends ReactiveVaultClient.RequestBodySpec, RequestHeadersPathSpec<RequestBodySpec> {
