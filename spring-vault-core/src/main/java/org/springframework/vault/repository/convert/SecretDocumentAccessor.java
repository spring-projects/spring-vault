...
/**
 * Wrapper value object for a {@link SecretDocument} to be able to access raw
 * values by {@link VaultPersistentProperty} references. The accessors will
 * transparently resolve nested document values that a
 * {@link VaultPersistentProperty} might refer to through a path expression in
 * field names.
 *
 * @author Mark Paluch
 * @since 2.0
...
class SecretDocumentAccessor {
	...
	/**
	 * Create a new {@code SecretDocumentAccessor} for the given
	 * {@link SecretDocument}.
	...
	/**
	 * Create a new {@code SecretDocumentAccessor} for the given
	 * {@link SecretDocument} and {@link Map body}.
	 * @param document must be a {@link SecretDocument} effectively, must not be
	 * {@literal null}
	 ...
	 * coordinates defined through the given {@link VaultPersistentProperty}. By
	 * default this will be the plain field name. But field names might also consist
	 * of path traversals so we might need to create intermediate {@link Map}s.
	...
	/**
	 * Return the value the given {@link VaultPersistentProperty} refers to. By
	 * default this will be a direct field but the method will also transparently
	 * resolve nested values the {@link VaultPersistentProperty} might refer to
	 * through a path expression in the field name metadata.
	...
	/**
	 * Return whether the underlying {@link SecretDocument} has a value
	 * ({@literal null} or non-{@literal null}) for the given
	 * {@link VaultPersistentProperty}.
	 * @param property must not be {@literal null}.
	 * @return
	 */
	...
	/**
	 * Return the {@link Map} which either already exists in the given source under
	 * the given key, or creates a new nested one, registers it with the source and
	 * returns it.
	 ...
	 * {@link VaultPersistentProperty}. Nested accessors allows mapping of
	 * structured hierarchies and represent accessors to nested maps.
	 * @param property must not be {@literal null}.
	 * @return
	 */
	@SuppressWarnings("unchecked")
