...
/**
 * Represents a requested secret from a specific Vault path associated with a
 * lease {@link Mode}.
 * <p>A {@link RequestedSecret} can be renewing or rotating.
 *
 * @author Mark Paluch
...
public class RequestedSecret {
	 ...
	 * @param path must not be {@literal null} or empty, must not start with a
	 * slash.
	...
	/**
	 * Create a rotating {@link RequestedSecret} at {@code path}. A lease associated
	 * with this secret will be renewed if the lease is qualified for renewal. Once
	 * the lease expires, a new secret with a new lease is obtained.
	 * @param path must not be {@literal null} or empty, must not start with a
	 * slash.
	...
	public enum Mode {
		...
		/**
		 * Renew lease of the requested secret. Obtains new secret along a new lease
		 * once the previous lease expires its max lease time.
		 */
		ROTATE;
