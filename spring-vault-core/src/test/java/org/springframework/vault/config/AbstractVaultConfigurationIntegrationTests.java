...
class AbstractVaultConfigurationIntegrationTests extends PkiIntegrationTestSupport {
	...
	void shouldRegisterManagedCertificate() {
		...
		AtomicReference<CertificateBundle> bundle = new AtomicReference<>();
		try (AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext()) {

			context.register(VaultIntegrationTestConfiguration.class);
			context.registerBean(ManagedCertificate.class, () -> {
				return ManagedCertificate.issue("foo", "testrole", VaultCertificateRequest.create("www.example.com"),
						bundle::set);
			});
			...
			assertThat(bundle).hasValueSatisfying(actual -> {
